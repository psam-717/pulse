...
### 3.9 Aza `amount` is GHS major units — the docs example is WRONG (P4, corrected Aug 30 2026)
- **When:** P4 (Aug 18 2026) — planning by two agents; CORRECTED Aug 30 2026 after
  empirical probe + engineer conversation.
- **Symptom:** charging ₵350.00 as `"amount": 35000` showed **GH₵ 35,000.00** on the
  hosted checkout and dashboard. The old guidance (below) was the cause.
- **Root cause (corrected):** Aza's public landing (aza.systems/developers) says
  "Charge ₵50.00" → `"amount": 5000` — pesewas. **That example is wrong.** Probe:
  `amount: 1` renders **GH₵ 1.00** on pay.aza.systems/c/… (pesewas would render
  ₵0.01). Aza's `amount` field is **GHS major units**; `"amount": 5000` actually
  charges ₵5,000.00. First reported to Aza by Pulse (bug-triage BE-5); their docs
  still show the incorrect example.
- **Fix:** convert exactly once in `AzaAmountConverter.toAzaAmount` — whole cedis,
  NO ×100. `Booking.amountDue` (BigDecimal GHS) → `long` as-is.
- **Prevention:** never trust a payment provider's landing-page example — probe
  with `amount: 1` and read the hosted checkout page before coding to the docs.
  Webhook payload amounts are not validated by `PaymentService.completeSession`
  (session-id lookup only), and `PaymentHistory.amount` comes from
  `Booking.amountDue`, so history/display stayed correct throughout.
- **OBSOLETE (pre-Aug 30):** "amount is pesewas; toMinorUnits(movePointRight(2))".
